...
async def student_graduations(student_id: str, user: dict = Depends(get_current_user)):
    ...
    docs = await db.graduations.find({"student_id": student_id}).sort("graduation_date", -1).to_list(500)
    result = []
    for d in docs:
        d.pop("_id", None)
        result.append(d)
...
async def current_belt(student_id: str, modality_id: str, user: dict = Depends(get_current_user)):
    if user["role"] == "student" and user.get("linked_id") != student_id:
        raise HTTPException(status_code=403, detail="Acesso negado")
    doc = await db.graduations.find({"student_id": student_id, "modality_id": modality_id}).sort("graduation_date", -1).to_list(1)
    if not doc:
        return None
    d = doc[0]
    d.pop("_id", None)
...
async def delete_graduation(graduation_id: str, user: dict = Depends(require_admin)):
    res = await db.graduations.delete_one({"id": graduation_id})
