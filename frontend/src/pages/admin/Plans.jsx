...
const empty = { name: "", value: 0, early_value: "", periodicity: "monthly", description: "", status: "active" };
...
export default function Plans() {
  ...
  const openEdit = (p) => { setEditing(p); setForm({ ...empty, ...p, early_value: p.early_value ?? "" }); setOpen(true); };
  ...
  const submit = async () => {
    try {
      const payload = {
        ...
        early_value: form.early_value === "" || form.early_value == null ? null : parseFloat(form.early_value),
      };
      if (editing) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {items.map(p => (
          <div key={p.id} className="card-flat p-6" data-testid={`plan-card-${p.id}`}>
            ...
            {p.early_value != null && (
              <div className="mt-4 border-l-2 border-emerald-600 pl-3">
                <div className="text-[9px] uppercase tracking-widest text-emerald-500">Até 5º dia útil</div>
                <div className="font-mono text-2xl text-emerald-400 mt-0.5">{brl(p.early_value)}</div>
              </div>
            )}
            <div className={`${p.early_value != null ? "mt-3 border-l-2 border-zinc-700 pl-3" : "mt-4"}`}>
              {p.early_value != null && <div className="text-[9px] uppercase tracking-widest text-zinc-500">Após 5º dia útil</div>}
              <div className={`font-mono ${p.early_value != null ? "text-xl text-white" : "text-3xl"} mt-0.5`}>{brl(p.value)}</div>
            </div>
            <div className="text-xs text-zinc-500 mt-3 min-h-[32px]">{p.description}</div>
        ...
        <DialogContent className="bg-[#121212] border-zinc-800 text-white rounded-none max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <F label="Até 5º dia útil (R$)"><Input data-testid="plan-early-value-input" type="number" step="0.01" placeholder="Opcional" value={form.early_value} onChange={(e) => setForm({ ...form, early_value: e.target.value })} /></F>
              <F label="Após 5º dia útil (R$) *"><Input data-testid="plan-value-input" type="number" step="0.01" value={form.value} onChange={(e) => setForm({ ...form, value: e.target.value })} /></F>
            </div>
            <div className="text-[11px] text-zinc-500 -mt-2">O sistema aplica automaticamente o valor com desconto se o pagamento for até o 5º dia útil do mês.</div>
